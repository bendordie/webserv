...
WebServer::WebServer(EventSelector *event_selector, int port, int fd, const Config &config, int index_from_config)
    : FdHandler(fd, true), _name("webserv"), _port(port), _indexInConfig(index_from_config),
      _config(config), _eventSelector(event_selector), _timeoutSec(35), _timeoutMutex(event_selector->getTimeoutMutex()) {

    _timeoutHandler = thread(&WebServer::checkSessionTimeout, this);
    _virtualServers.insert(make_pair("server_name", VirtualServer(config, index_from_config)));
    initContentTypes();
...
bool WebServer::addNewVirtualServer(const Config &config, int index_in_config) {

    if (!config.isTraitDefined(index_in_config, "server_name")) {
        cerr << "Error: Field server_name is not defined for Server " << index_in_config << endl;
        return false;
    }

    string server_name = config[to_string(index_in_config) + ".server.server_name"];
    if (server_name.empty()) {
        cerr << "Error: Field server_name is not defined for Server " << index_in_config << endl;
        return false;
    }

    _virtualServers.insert(std::make_pair(server_name, VirtualServer(config, index_in_config)));
...
WebServer *WebServer::init(EventSelector *eventSelector, int port, const Config &config, int indexInConfig) {
    ...
    return new WebServer(eventSelector, port, listen_socket, config, indexInConfig);
}
...
void WebServer::handle(bool read, bool write) {
    ...
    auto *session = new WebSession(this, client_socket);
...
void WebServer::initResponseStatuses() {
    _responseStatuses.insert(std::make_pair(100, "Continue"));
    _responseStatuses.insert(std::make_pair(301, "Moved Permanently"));
    _responseStatuses.insert(std::make_pair(400, "Bad Request"));
    _responseStatuses.insert(std::make_pair(403, "Forbidden"));
    _responseStatuses.insert(std::make_pair(404, "Not Found"));
    _responseStatuses.insert(std::make_pair(405, "Not Allowed"));
    _responseStatuses.insert(std::make_pair(500, "Internal server error"));
}

const ServerOptions *WebServer::getLocationOptions(const string &server_name, const string &url) const {

    auto virtual_server = _virtualServers.find(server_name);
    if (virtual_server == _virtualServers.end()) {
        const VirtualServer &default_server = _virtualServers.begin()->second;
        return default_server.getLocationOptions(url);
    }

    return virtual_server->second.getLocationOptions(url);
}

const string &WebServer::getName() const { return _name; }

int WebServer::getPort() const { return _port; }

const map<string, string> & WebServer::getContentTypes() const { return _contentTypes; }

const map<int, string> & WebServer::getResponseStatuses() const { return _responseStatuses; }